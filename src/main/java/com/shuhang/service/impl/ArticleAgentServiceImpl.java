...
@Service
@Slf4j
public class ArticleAgentServiceImpl extends ServiceImpl<ArticleMapper, Article> implements ArticleAgentService {
    ...
    public void executeArticleGeneration(ArticleState state, Consumer<String> streamHandler){
        try {
            ...
            // 智能体2：生成大纲（流式输出），报错：SSE Emitter 不存在, taskId=60200047c80a4459b778e2e1e9df44ba
    ...
    private ArticleState.ImageResult buildImageResult(ArticleState.ImageRequirement requirement,
        ...
        imageResult.setPlaceholderId(requirement.getPlaceholderId());   // 记录占位符 Id
        return imageResult;
    }
